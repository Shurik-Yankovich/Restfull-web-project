package electronicbookstore.menu.constant;

public class OrderMenuTextConst {

    public static final String ITEM_TEXT_ADD_ORDER = "Добавить книгу заказ";
    public static final String ITEM_TEXT_CANCEL_ORDER = "Отменить заказ";
    public static final String ITEM_TEXT_COMPLETE_ORDER = "Выполнить заказ";
    public static final String ITEM_TEXT_SHOW_ORDER_LIST = "Показать список заказов";
    public static final String ITEM_TEXT_SHOW_COMPLETED_ORDER_LIST = "Показать список выполненных заказов за период";
    public static final String ITEM_TEXT_SHOW_ORDER_DESCRIPTION = "Показать детали заказа";
    public static final String ITEM_TEXT_BACK = "Вернутся в главное меню";


    private OrderMenuTextConst() {
    }
}
